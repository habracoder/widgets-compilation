...
var widget = JSON.parse("{\"id\":12,\"name\":\"23123\"}");

MarketGidBaseBlockC = function MarketGidBaseBlockC(root_id, context, fallback, containerId, uniqId, versionWidget) {
  var self = this;
  ...
  this.MarketCutStr = function (str, limit, dots) {
    ...
    for (i = 1; i < word.length; i++) {
      test = ret + word[i];
      if (test.length > limit) return ret + (typeof dots == 'undefined' || dots ? '...' : '');else ret += word[i] + ' ';
    }
  ...
  this.prepareTitle = function (title) {
    title = this.MarketParseStr(title, 15);
    title = this.MarketCutStr(title, 60, 1);
    return title;
  };

  this.prepareDesc = function (desc) {
    desc = this.MarketParseStr(desc, 15);
    desc = this.MarketCutStr(desc, 60);
    return desc;
  };
    ...
    if (typeof servicerData != 'undefined') {
      ...
      if (self.servicerData.hasOwnProperty('templateId') && self.templates[self.servicerData.templateId]) {
        ...
        var getTemplate = function getTemplate() {
          var i,
              templates = self.context._mgtemplates;
  ...
  this.renderItem = function (n, callbackText, type) {
    ...
    if (self.isArray(n)) {
      ...
      if (p.id && (!(p.id in self.blockIds) || typeof self.isElastic !== 'undefined' && self.isElastic == true)) {
        ...
        var createWebLink = function createWebLink(url) {
        ...
        var vars = [[/\{\*.*?\*\}/, ''], [/\{\$href\}/g, self.hrefAttr + '="' + self.prepareNiceHref(p.hash) + '" data-hash="' + p.hash + '"'], [/\{\$pmc_item\}/, self.id], [/\{\$hash\}/g, p.hash], [/\{\$target\}/g, typeof p.other.type !== 'undefined' && p.other.type == 'i' || typeof self.servicerData.ats !== 'undefined' && self.servicerData.ats ? 'target="_top"' : 'target="_blank"'], [/(class\=\"[^+]?)(mgline)([^+]?\")/, '$1$2 ' + addClasses.join(' ') + '$3'], [/\{\$source\}/g, p.source], [/\{\$src\}/g, 'width="90" height="90"' + ' data-i="' + p.id + '" src="' + createWebLink(p.other["i"]) + '"'], [/\{\$title\}/g, self.prepareTitle(p.title)], [/\{\$desc\}/g, self.prepareDesc(p.desc)], [/\{\$iteration\}/g, self.iteration], [/\$iteration/g, self.iteration], [/this\.iteration/g, self.iteration], [/\{\$price\}/g, p.price], [/\{\$priceold\}/g, p.priceold], [/\{\$discount\}/g, p.discount], [/\{if \$price\}/g, "{if '' != " + price + "}"], [/\{if !\$price\}/g, "{if '' == " + price + "}"], [/\{if \$priceold\}/g, "{if '' != " + priceOld + "}"], [/\{if \$price != " "\}/g, "{if " + p.price.trim().length + " != 0}"], [/\{if \$price == " "\}/g, "{if " + p.price.trim().length + " == 0}"], [/\$priceold/g, p.priceold], [/\$price/g, p.price], [/\{if \$rep\}/g, "{if p.isReplic}"], [/\{\$banner\}/g, banner], [/\{if \$teaser\}/g, "{if '' == '" + banner + "'}"], [/\{if \$banner\}/g, "{if '' != '" + banner + "'}"], [/\{\$category\}/g, p.other.category || ''], [/\{\$sourceName\}/g, self.sourceName]];
  ...
  this.injectionScriptMethod = function (forceShow, refresh) {
    ...
    if (JSON.parse('[]').indexOf('int_exchange') >= 0 || 0) {
      ...
      if (ogUrlEl) {
        self.requestParams.ogurl = 'ogurl=' + encodeURIComponent(_typeof(ogUrlEl) === "object" ? ogUrlEl.getAttribute('content') : ogUrlEl);
      }
  ...
  this.adBlockDetect = function () {
    ...
    var createBlockDetectionDiv = function createBlockDetectionDiv(className) {
    ...
    var isBlockDetectedOnDiv = function isBlockDetectedOnDiv(e) {
      return !e.offsetHeight;
    };

    var isBlockDetectedOnClassNames = function isBlockDetectedOnClassNames(e) {
      var i,
          r = e.length,
          o;
    ...
    var getBlockedState = function getBlockedState(e) {
...
if ((void 0)['mgCanLoad605']) {
  if (!mgFallback605) {
    ...
    if (!window.document.getElementById("MarketGidComposite605") || !window.document.getElementById("M180Composite605")) {
  ...
  (void 0)['MarketGidCMainBlock605'] = function (self) {
    /*  */
  ...
  (void 0)['MarketGidCMainBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Main'] = 'MarketGidCMainBlock605';

  MarketGidCCookieBlock605 = function MarketGidCCookieBlock605(self) {
  ...
  (void 0)['MarketGidCCookieBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Cookie'] = 'MarketGidCCookieBlock605';

  MarketGidCSubnetsBlock605 = function MarketGidCSubnetsBlock605() {
    var self = this;
    ...
    this.subnetDashboardDomains['0'] = self.webProtocol + '//dashboard.marketgid.com';
    this.subnetMirrors['1'] = 'ladycenter.ru';
    ...
    this.subnetDashboardDomains['1'] = self.webProtocol + '//dashboard.marketgid.com';
    this.subnetMirrors['2'] = 'tovarro.com';
    ...
    this.subnetDashboardDomains['2'] = self.webProtocol + '//dashboard.tovarro.com';
    ...
    this.subnetDashboardDomains['3'] = self.webProtocol + '//dashboard.lentainform.com';
    this.subnetMirrors['4'] = 'marketgid.com';
  ...
  (void 0)['MarketGidCSubnetsBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Subnets'] = 'MarketGidCSubnetsBlock605';

  MarketGidCInternalExchangeLoggerBlock605 = function MarketGidCInternalExchangeLoggerBlock605(self) {
  ...
  (void 0)['MarketGidCInternalExchangeLoggerBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['InternalExchangeLogger'] = 'MarketGidCInternalExchangeLoggerBlock605';
  /**
  ...
  MarketGidCInternalExchangeBlock605 = function MarketGidCInternalExchangeBlock605(self) {
  ...
  (void 0)['MarketGidCInternalExchangeBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['InternalExchange'] = 'MarketGidCInternalExchangeBlock605';

  (void 0)['MarketGidCAntiAdblockBlock605'] = function (self) {
  ...
  (void 0)['MarketGidCAntiAdblockBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['AntiAdblock'] = 'MarketGidCAntiAdblockBlock605';

  MarketGidCUtilsBlock605 = function MarketGidCUtilsBlock605(self) {
    self.utils = {};
    /**
  ...
  (void 0)['MarketGidCUtilsBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Utils'] = 'MarketGidCUtilsBlock605';

  MarketGidCRtbBlock605 = function MarketGidCRtbBlock605(self) {
  ...
  (void 0)['MarketGidCRtbBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Rtb'] = 'MarketGidCRtbBlock605';

  MarketGidCMgqBlock605 = function MarketGidCMgqBlock605(self) {
  ...
  (void 0)['MarketGidCMobileDoubleClickBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['MobileDoubleClick'] = 'MarketGidCMobileDoubleClickBlock605';

  MarketGidCDesktopDoubleClickBlock605 = function MarketGidCDesktopDoubleClickBlock605(self) {
    var isTouch = 'ontouchstart' in window || navigator.msMaxTouchPoints > 0;
    ...
    if (!isTouch && isGoodBrowser) {
      ...
      this.informerId = this.root.getAttribute('id');
      self.styles += '.MarketGidDButton605{' + 'display:       inline-block;' + 'width:         168px;' + 'height:        33.33%;' + 'background:    -moz-linear-gradient(#7DC586, #5BA860);' + 'background:    -o-linear-gradient(#7DC586, #5BA860);' + 'background:    -webkit-linear-gradient(#7DC586, #5BA860);' + 'background:    linear-gradient(#7DC586, #5BA860);' + 'border:        1px solid #3C9342;' + 'color:         #fff !important;' + 'font:          normal 700 21px/48px "Zapf Dingbats", sans-serif;' + 'opacity: 0;' + 'left: -100%;' + 'text-align:    center;' + 'text-shadow:   none;' + 'position: absolute;' + 'text-decoration: none !important;' + 'transition:all 1s;' + '}' + '.MarketGidDLayout605{' + 'height: 100%;' + 'width: 100%;' + 'position: absolute;' + 'top: 0px;' + 'left: 0px;' + 'background-color: rgba(0, 0, 0, 0.5);' + 'z-index: 100;' + '}' + '#MarketGidComposite605 .mgline {position:relative}' + '.MarketGidDButton605:before{' + 'content: "";' + 'display: inline-block;' + 'vertical-align: middle;' + 'opacity: 0;' + 'visibility: hidden;' + 'height: 100%;' + 'width: 0;}';
    }
  ...
  (void 0)['MarketGidCDesktopDoubleClickBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['DesktopDoubleClick'] = 'MarketGidCDesktopDoubleClickBlock605';
  /**
   * Блок антинакрутки
   * @param self
   */

  (void 0)['MarketGidCAntifraudBlock605'] = function (self) {
    ...
    self.afDeepSession = '';
    self.afOrigHref = '';
    /* переменная хранит ссылку, по которой произошел клик. */

    self.afOrigHrefLink = '';
    /* переменная хранит изначальный адрес ссылки, по которой произошел клик. */

    self.afMouseMoves = '';
    /* строка, которая хранит траекторию передвижения курсора мыши (в виде координат), если было произведено смещение информера.  */

    self.afMgLoaded = 0;
    self.afImgLoaded = 0;
    self.afSnapshotProperties = '';
    /* снимок DOM-свойств, который нужен для определения группы браузера */

    self.afCoords = '';
    /* координаты информера в формате h|w|x|y */
    ...
    self.initAntiFraud = function () {
      ...
      self.afCookieData = d.getTime() + '' + self.id + Math.floor(Math.random() * 100) + '' + 2 * Math.floor(Math.random() * 4);
      self.afCookieData += 1;
      self.afCookieData = self.compressNumber(self.trimString(self.afCookieData, 16, 1));
      self.afMgLoaded = self.compressNumber(d.getTime());
    ...
    self.isFlashEnabled = function () {
      ...
      if (typeof self.context.navigator.plugins !== 'undefined' && _typeof(self.context.navigator.plugins["Shockwave Flash"]) == 'object') {
        b = 1;
    ...
    self.getSnapshotProperties = function () {
      var propertiesDom = {
        "props": {
          "3": "window.close",
          "13": "window.navigator.taintEnabled",
          "16": "window.EventTarget.toString",
          "18": "window.navigator.appName",
          "20": "window.ApplicationCacheErrorEvent",
          "22": "window.clearImmediate",
          "23": "window.CryptoKey",
          "25": "window.external.AddSearchProvider",
          "32": "window.chrome.search",
          "33": "window.clientInformation.vendor",
          "40": "window.TouchEvent",
          "43": "window.DeviceMotionEvent",
          "46": "window.navigator.webkitGetGamepads",
          "49": "window.Notification",
          "56": "window.BeforeLoadEvent",
          "57": "window.Entity",
          "72": "window.AnalyserNode",
          "74": "window.ArrayBufferView",
          "76": "window.HTMLBaseFontElement",
          "86": "window.self.SharedWorker",
          "87": "window.Touch",
          "91": "window.DOMException.ABORT_ERR",
          "92": "window.Set",
          "98": "window.document.createTouch",
          "138": "window.NaN",
          "139": "window.document.prepend",
          "163": "window.SiteBoundCredential",
          "165": "window.CanvasCaptureMediaStreamTrack",
          "166": "window.core.version",
          "167": "window.Blob"
        },
        "rules": {
          "contains": {
            "3": ["({command:\"window.close.self\"})"],
            "18": ["MicrosoftInternetExplorer"],
            "33": ["GoogleInc."]
          },
          "not_empty": [13, 16, 18, 20, 22, 23, 25, 32, 40, 43, 46, 49, 56, 57, 76, 86, 87, 91, 92, 98, 138, 139, 163, 165, 166],
          "undefined": [25, 46, 72, 74, 87, 167]
        }
      };
    ...
    self.getAntifraudParams = function (event, element) {
      ...
      for (var i = 1; i <= Math.ceil((self.afBrowserFlag + 1) / 68) * 48; i++) {
        ...
        switch (i) {
          ...
          case 16:
            /* вертикальная координата (в соответствии с клиентской областью) указателя мыши */
          ...
          case 28:
            /* поддержка java, flash, cookie, localStorage, sessionStorage */
          ...
          case 43:
            /* координата x относительно экрана  */
            paramPart = self.compressNumber(self.getScreenX());
            break;

          case 44:
            /* координата y относительно экрана  */
          ...
          case 48:
            /* пользователь имеет статус online */
  ...
  (void 0)['MarketGidCAntifraudBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Antifraud'] = 'MarketGidCAntifraudBlock605';

  MarketGidCDiscountBlock605 = function MarketGidCDiscountBlock605(self) {
  ...
  (void 0)['MarketGidCDiscountBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Discount'] = 'MarketGidCDiscountBlock605';

  MarketGidCCountersBlock605 = function MarketGidCCountersBlock605(self) {
  ...
  (void 0)['MarketGidCCountersBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Counters'] = 'MarketGidCCountersBlock605';

  (void 0)['MarketGidCAdvertLinkBlock605'] = function (self) {
  ...
  (void 0)['MarketGidCAdvertLinkBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['AdvertLink'] = 'MarketGidCAdvertLinkBlock605';
  /**
   * Блок кнопки блокировки тизера в информере.
   * @param self
   */

  (void 0)['MarketGidCRejectBlock605'] = function (self) {
  ...
  (void 0)['MarketGidCRejectBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Reject'] = 'MarketGidCRejectBlock605';
  /**
   * VT-21966 Сторонние пиксели в js-коде информеров
   * @param self
   */

  (void 0)['MarketGidCExternalCountersBlock605'] = function (self) {
    self.linksBlocked;
  ...
  (void 0)['MarketGidCExternalCountersBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['ExternalCounters'] = 'MarketGidCExternalCountersBlock605';

  MarketGidCCriteoBlock605 = function MarketGidCCriteoBlock605(self) {
    self.context.LoadCriteoAllPlaces = function (params) {
      ...
      self.context.Criteo.events.push(function () {
        self.context.Criteo.Passback.RenderAd({
          adUnit: self.root.id,
          passback: function passback(adunit) {}
        });
      });
    };
  ...
  (void 0)['MarketGidCCriteoBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Criteo'] = 'MarketGidCCriteoBlock605';

  MarketGidCSendDimensionsBlock605 = function MarketGidCSendDimensionsBlock605(self) {
    ...
    self.updatePrecalcRect = function (callback) {
      ...
      setTimeout(function () {
        ...
        for (i = 0; i < elx.length; i++) {
          ...
          if (typeof prepTop != "undefined" && prepTop < rect.top && cols === 0) {
            cols = i;
          }
  ...
  (void 0)['MarketGidCSendDimensionsBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['SendDimensions'] = 'MarketGidCSendDimensionsBlock605';

  MarketGidCMonitorBlock605 = function MarketGidCMonitorBlock605(self) {
    ...
    this.monitorInit = function () {
      ...
      if (typeof IntersectionObserver === 'function' && self.servicerData.cv === 3) {
        ...
        var setIntersectingTime = function setIntersectingTime() {
          var shown = 0;
        ...
        if (!self.intersectionObserver) {
          ...
          self.intersectionObserver = new IntersectionObserver(callback, {
        ...
        var collectCallback = function collectCallback() {
          collectTimeout = null;
        ...
        var intersectionObserverCollect = function intersectionObserverCollect(obj) {
    ...
    this.prepareCappingData = function (blocks, pv) {
      ...
      for (var i in blocks) {
        ...
        if (showHash && self.teaserData[showHash]) {
          ...
          if (self.teaserData[showHash].ch && self.teaserData[showHash].ccid && self.teaserData[showHash].cdomain) {
    ...
    this.sendCappingData = function (dataQuery, domain) {
      ...
      if (ispv2) {
        if (typeof self.context._mgwcapping == 'undefined' || self.context._mgwcapping.indexOf(self.id.toString()) == -1 || typeof cid != "undefined" && self.context._mgwcapping.indexOf(cid.toString()) == -1) {
  ...
  (void 0)['MarketGidCMonitorBlock605'].call((void 0)['MarketGidInfC605'], (void 0)['MarketGidInfC605']);
  (void 0)['MarketGidInfC605']['funcBlocks']['Monitor'] = 'MarketGidCMonitorBlock605';
